...
const Trainings = () => {
    ...
    const [training_tasks_array, setTraining_tasks] = useState([])
    ...
    let category_array = ['Training']

    const [training_checked_count, setTraining_checked_count] = useState()
    const [totalTrainingTasks, setTotalTrainingTasks] = useState()
...
const getTaskInput = (e) => {
    if(e.key === 'Enter') {
        ...
        if(category === 'Training') {
            setTraining_tasks([...training_tasks_array, newTask ])
            // localStorage.setItem("stored_navbar_array", training_tasks_array);
            console.log( 'navbar array',training_tasks_array)
        }
...
const getChecked = () => {
    ...
    if(category === 'Training') {
        var checkbox = document.getElementById("trainings_check")
        if(checkbox.checked == true){
            console.log('i am checked')
            var $boxes = $('input[name=training_check]:checked');
            var box_count = $boxes.length;
    
            console.log('number checked',box_count)
            console.log('total number of tasks',training_tasks_array.length)
            var total_nav_tasks = training_tasks_array.length
            setTotalTrainingTasks(total_nav_tasks)
    
            var percent = ((box_count/total_nav_tasks) * 100).toFixed(2)
            setTraining_checked_count(percent)
        }
    ...
    let mean_array =[training_checked_count]
  ...
  return (
    <div className='homepage'>
        ...
        <p className="home_tasks font-sans text-base text-blue-900 font-bold">Training Tasks</p>
        ...
        <input type="text" name="" id="task_input" onKeyDown={getTaskInput} value={inputText}  onChange={(e) => setInputText(e.target.value)} />
            ...
            <div className="trainings_cards">
            
            <div className="training_card">

                <p className="font-sans text-base text-blue-900 font-semibold mx-10 my-6">Trainings</p>
                {
                    category === 'Training' ? 
                    
                        training_tasks_array.map( (task, index) => 
                            
                            <div className="flex flex-row gap-2 p-3 -my-6 mx-6">
                                <input type="checkbox" name="training_check" id="trainings_check" onClick={getChecked} />
            ...
            <div className="training_complete_rate">
